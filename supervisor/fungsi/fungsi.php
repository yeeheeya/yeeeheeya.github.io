...
// --------------------------------------------ADMIN SECTION-----------------------------------------------------------------------------
function panggil_supervisor()
{
	global $koneksi;
	$id  = $_SESSION['idabsen'];
	return mysqli_query($koneksi, "SELECT * FROM tb_supervisor WHERE id='$id'");
}

function select_supervisor()
{
	global $koneksi;
	return mysqli_query($koneksi, "SELECT * FROM tb_supervisor ORDER BY id DESC");
}

function select_admin_2()
{
	global $koneksi;
	$select = mysqli_query($koneksi, "SELECT count(id) AS jadmin FROM tb_supervisor");
	$r = mysqli_fetch_array($select);
	echo $r['jadmin'];
}

function simpan_supervisor()
{
	global $koneksi;
	...
	if ($foto!= "") {
		...
		if (in_array($ext, $allowed_ext)===true) {
			...
			$res = mysqli_query($koneksi, "INSERT INTO tb_supervisor SET username='$username', password='$password', nama='$nama', kontak='$kontak', foto='$nama_file_baru'");

		}
	}else if (empty($_POST['foto'])) {
		$res = mysqli_query($koneksi, "INSERT INTO tb_supervisor SET username='$username', password='$password', nama='$nama', kontak='$kontak'");
	}
...
function hapus_admin()
{
	global $koneksi;
	$id = $_POST['id'];
	$select = mysqli_query($koneksi, "SELECT * FROM tb_supervisor WHERE id='$id'");
	$array = mysqli_fetch_array($select);
	$foto = $array['foto'];

	if ($array['foto'] == "") {
		return mysqli_query($koneksi, "DELETE FROM tb_supervisor WHERE id='$id'");
	}else{
		unlink("img/$foto");
		return mysqli_query($koneksi, "DELETE FROM tb_supervisor WHERE id='$id'");
	}
...
function edit_admin()
{
	...
	// unlink 
	$sql = mysqli_query($koneksi, "SELECT * FROM tb_supervisor WHERE id='$id'");
				...
				$angka_acak = rand(1,999);
		   		$nama_file_baru = $angka_acak.'-'.$foto;
		   		    if (in_array($ekstensi, $allowed_ext) === true) {
		      			move_uploaded_file($file_tmp, 'img/'.$nama_file_baru);
		      			$result =  mysqli_query($koneksi, "UPDATE tb_supervisor SET username='$username', password='$password', nama='$nama', kontak='$kontak', foto='$nama_file_baru' WHERE id='$id'");
	...
	if (empty($_POST['foto'])) {
		return  mysqli_query($koneksi, "UPDATE tb_supervisor SET username='$username', password='$password', nama='$nama', kontak='$kontak' WHERE id='$id'");
	}
